...
namespace MB
{
    public class WebView : IDisposable
    {
        ...
        public WebView(IntPtr windowHwnd, bool isTransparent = false)
        {
            /*if (!File.Exists($"{Environment.CurrentDirectory}\\node.dll"))
            {
                MessageBox.Show("请在程序同目录下放置node.dll文件", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        /// <returns></returns>
        public bool Bind(IntPtr windowHwnd, bool isTransparent = false)
        {
            if (m_hWnd == windowHwnd)
            {
                return true;
            }
